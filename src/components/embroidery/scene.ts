import { FABRIC, WORDS, type WordPatch } from "#/components/embroidery/patches";
...
/**
 * CPU pass. Builds two textures:
 *
 * - ART (RGB): the fabric, then per word — the dilated silhouette filled with
 *   cloth colour, the white merrowed border band, and the dark ink glyphs,
 *   composited front-to-back so a nearer patch hides the ink of ones behind it.
 * - FIELD (RGBA): R = blurred coverage / puff ramp, G = ink mask, B = border
 *   ring, A = per-pixel satin-stitch run angle.
 *
 * The run angle is the realism cue: it comes from the gradient of a heavily
 * blurred glyph field so stitches run ALONG each stroke, and from the coverage
 * rim on the border so they WRAP it like a real merrow.
 */
export async function makeScene(
	...
	const cover = mk();
	const coverX = cover.getContext("2d") as CanvasRenderingContext2D;
	const ink = mk();
	const inkX = ink.getContext("2d") as CanvasRenderingContext2D;
	const ring = mk();
	const ringX = ring.getContext("2d") as CanvasRenderingContext2D;
	const dir = mk();
	const dirX = dir.getContext("2d") as CanvasRenderingContext2D;

	const art = mk();
	const artX = art.getContext("2d") as CanvasRenderingContext2D;
	...
	for (const wp of WORDS) {
		const glyph = mk();
		const gx = glyph.getContext("2d") as CanvasRenderingContext2D;
		drawWord(gx, wp, W, H, fontFamily);

		const sil = mk();
		const sx = sil.getContext("2d") as CanvasRenderingContext2D;
		dilate(sx, glyph, borderPx * 2.1);

		const inner = mk();
		const ix = inner.getContext("2d") as CanvasRenderingContext2D;
		dilate(ix, glyph, borderPx * 1.05);

		const bandC = mk();
		const bx = bandC.getContext("2d") as CanvasRenderingContext2D;
	...
	const puff = mk();
	const px = puff.getContext("2d") as CanvasRenderingContext2D;
	px.filter = `blur(${bevelPx.toFixed(2)}px)`;
	px.drawImage(cover, 0, 0);
	px.filter = "none";

	const gsrc = mk();
	const gx2 = gsrc.getContext("2d") as CanvasRenderingContext2D;
	gx2.filter = `blur(${Math.max(2, H * 0.02).toFixed(2)}px)`;
	gx2.drawImage(dir, 0, 0);
	gx2.filter = "none";

	const rsrc = mk();
	const rx2 = rsrc.getContext("2d") as CanvasRenderingContext2D;
...
function paintMasked(
	...
	t.width = W;
	t.height = H;
	const tx = t.getContext("2d") as CanvasRenderingContext2D;
...
/** Cheap morphological grow: stamp the glyph at ringed offsets. */
function dilate(
	ctx: CanvasRenderingContext2D,
