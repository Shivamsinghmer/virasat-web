...
interface Props {
	...
	/**
	 * Element to listen on. Defaults to the whole document; pass a ref to scope
	 * the effect to one surface (the effect layer itself is pointer-events:none,
	 * so it cannot be the listener).
	 */
	target?: RefObject<HTMLElement | null>;
}

const SNIPER_ARMS = [0, 90, 180, 270];
const BURST_ARMS = [45, 80, 115, 150];
const CROSSHAIR_ARMS = [0, 90, 180, 270];
const WAVY_ARMS = [45, 90, 135, 180];
const SNIPER_SHARDS = [
	Math.PI / 3,
	(2 * Math.PI) / 3,
	(4 * Math.PI) / 3,
...
export default function MouseEffects({
	...
	useEffect(() => {
		const listenOn: Document | HTMLElement = target?.current ?? document;
		...
		listenOn.addEventListener("click", handleClick);
		return () => listenOn.removeEventListener("click", handleClick);
	}, [interactionMode, effectSize, target]);
					...
					<svg
						...
						ref={(el) => {
							...
							gsap
								...
								.to(
									el,
									{ opacity: 0, duration: duration * 0.2, ease: "linear" },
									duration * 0.8,
								);
						}}
					>
					...
					<svg
						...
						ref={(el) => {
							...
							lines.forEach((line, index) => {
								const angle = BURST_ARMS[index] * (Math.PI / 180);
				...
				crosshairs.map((crosshair) => (
					<svg
						...
						ref={(el) => {
							...
							const lines = el.querySelectorAll("line");
							lines.forEach((line, index) => {
								const angle = CROSSHAIR_ARMS[index] * (Math.PI / 180);
								const centerX = effectSize / 2;
								const centerY = effectSize / 2;
								...
								gsap.set(line, {
									attr: { x1: startX, y1: startY, x2: centerX, y2: centerY },
									strokeWidth,
								});
								gsap
									.timeline()
					...
					<div key={sniper.id}>
						<svg
							...
							ref={(el) => {
								...
								const lines = el.querySelectorAll("line");
								lines.forEach((line, index) => {
									const angle = SNIPER_ARMS[index] * (Math.PI / 180);
									const centerX = effectSize / 2;
									const centerY = effectSize / 2;
